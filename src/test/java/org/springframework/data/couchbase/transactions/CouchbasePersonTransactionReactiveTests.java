...
public class CouchbasePersonTransactionReactiveTests extends JavaIntegrationTests {
	...
	public void beforeEachTest() {
		...
		operations.findByQuery(EventLog.class).withConsistency(REQUEST_PLUS).all().collectList()
				.block();
	...
	static class PersonService {
		...
		public Mono<Person> savePersonErrors(Person person) {
			...
			return operations.insertById(Person.class).one(person) //
		...
		public Flux<EventLog> saveWithLogs(Person person) {
			...
			return Flux.merge(operations.save(new EventLog(new ObjectId().toString(), "beforeConvert")), //
					...
					.thenMany(operations.findByQuery(EventLog.class).all()) //
					.as(transactionalOperator::transactional);
		}
