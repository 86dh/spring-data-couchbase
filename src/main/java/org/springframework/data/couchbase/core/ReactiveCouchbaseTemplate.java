...
public class ReactiveCouchbaseTemplate implements ReactiveCouchbaseOperations, ApplicationContextAware {
	...
	public ReactiveCouchbaseTemplate(final ReactiveCouchbaseClientFactory clientFactory, final CouchbaseConverter converter) {
...
//	}
	...
	RuntimeException potentiallyConvertRuntimeException(final RuntimeException ex) {
		RuntimeException resolved = exceptionTranslator != null ? exceptionTranslator.translateExceptionIfPossible(ex) : null;
	...
	protected Mono<ReactiveCouchbaseTemplate> doGetTemplate() {
		return ReactiveCouchbaseClientUtils.getTemplate(clientFactory, SessionSynchronization.ON_ACTUAL_TRANSACTION, this.getConverter());
	}
...
*/
	...
	/**
	 * {@link CouchbaseTemplate} extension bound to a specific {@link ClientSession} that is applied when interacting with the
	 * server through the driver API.
	 * <br />
	 * The prepare steps for {} and {} proxy the target and invoke the desired
